...
### M-04 域名 + ICP 备案 ⚠️ 关键路径最长

| 字段 | 内容 |
|---|---|
| Owner | 项目方负责人 + 法务 |
| 输入 | 公司营业执照扫描 + 法人身份证扫描 + 负责人手机号(接审核电话)+ 公章 |
| 产出 | 域名(如 `yaoqi.com`)所有权 + ICP 备案号挂域名 + DNS 接管(指向阿里云 DNS) |
| 模板 | 见本文 §5 ICP 备案材料清单 |
| DoD | `dig api.yaoqi.<host>` 返阿里云 SLB IP + 工信部备案查询能查到 |
| 状态 | ⚪ |
| 风险 | **7-20 天**;**先动**,否则 M4 上不了 prod |
...
| 适用场景 | 通用 |
| 签名来源 | "企事业单位的全称或简称" |
| 证明文件 | 营业执照(同 ICP 那份)+ "曜齐"商标证(若有)/ 或 ICP 备案截图(无商标时) |
...
|---|---|---|---|---|
| M-01 阿里云主账号 | 🟢 | 完成实名 | — | — |
| M-04 域名+ICP | 🟡 | 提交至管局 | 等审核 | 管局电话核验未接到 |
| M-05 短信签名 | 🟡 | 提交"曜齐" | 等审核 | — |
| ... | ... | ... | ... | ... |
```
