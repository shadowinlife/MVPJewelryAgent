...
> 状态:🟡 进行中(Stage 1 完成 2026-05-24,Stage 2 完成 2026-05-24,Stage 3 完成 2026-05-26,Stage 4 进行中 2026-06-01 起)
...
3. **LLM** = prod: Azure OpenAI @ HK;dev/staging: **DashScope qwen3.7-max**(多模态);后端 `LLMClient` Protocol 动态切换 + admin 配置页;跨云公网直连。
...
| **Stage 4: API + Integrations** | 路由 stub(auth/cases/reports/files/ocr/memberships)+ JWT + RBAC deps + DAO/Service 层 + `LLMClient` Protocol + OSS/OCR/短信 client + `/health` 扩 redis/oss/aoai + Seed(super_admin / 字典) | 🟡 进行中 (2026-06-01 起) | 前端 mock route 全关;`docker build` + gitleaks + pip-audit + Trivy 全绿;Backend-Architecture §15 全部勾选 |
...
- 2026-05-26 #Stage3 #1-#5 7 tier Pydantic schemas + `crop_report_for_user` 唯一裁剪入口 + 8 条 RBAC 红线锁定 + 54 测试全绿(新增 27)+ ruff/mypy --strict clean / commit [c282855](.)
- 2026-06-01 #Stage4-LLM 多 Provider LLM 抽象(LLMClient Protocol + DashScope adapter + Azure adapter + Fernet 加密 + factory + admin 配置页 API + 前端 form)+ DashScope qwen3.7-max 端到端连通验证 + Alembic 0002 + 51 测试(新增 7 LLM)/ commit [39c7c32](.)
