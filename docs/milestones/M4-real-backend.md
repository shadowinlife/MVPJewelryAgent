...
> 状态:🟡 进行中(Stage 1 完成 2026-05-24,Stage 2-4 待启动)
...
| **Stage 2: Persistence** | 13 张 ORM + Alembic 初始迁移 + testcontainers fixture + DAO 层;`/health` 扩 `checks.db` | ⚪ 未开始 | `pytest` 用真 PG 跑迁移;表结构与 [Backend-Database-Schema §核心表](../Backend-Database-Schema_v0.1.md) 严格对齐 |
| **Stage 3: Tier Schemas** | 7 个 tier 的 Pydantic schema + `cropReportForUser` 服务端裁剪服务 + 客户简洁版字段集 | ⚪ 未开始 | 8 条 RBAC 红线 + UI-Spec §17.3 数据红线测试全绿 |
| **Stage 4: API + Integrations** | 路由 stub(auth/cases/reports/files/ocr/memberships)+ JWT + RBAC deps + `LLMClient` Protocol + OSS/OCR/短信 client + `/health` 扩 db/redis/oss/aoai | ⚪ 未开始 | 前端 mock route 全关;`docker build` + gitleaks + pip-audit + Trivy 全绿;Backend-Architecture §15 全部勾选 |
